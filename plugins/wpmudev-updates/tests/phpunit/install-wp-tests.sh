...
if [ $# -lt 3 ]; then
	echo "usage: $0 <db-name> <db-user> <db-pass> [db-host] [wp-version] [reload-core] [reload-test-suite] [remove-database] [skip-database-creation]"
	exit 1
fi
...
WP_VERSION=${5-latest}
RELOAD_CORE=${6-false}
RELOAD_TEST_SUITE=${7-false}
REMOVE_DB=${8-false}
SKIP_DB_CREATE=${9-false}
...
install_wp() {
	...
	if [ -d $WP_CORE_DIR ]; then
        # if reload test suite, delete test suite directory
...
install_test_suite() {
    ...
    # if reload test suite, delete test suite directory
	if [ ${RELOAD_TEST_SUITE} = "true" ]; then
